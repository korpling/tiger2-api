...
public class LoadingTest_TigerXML extends TestCase {
	...
	/**
	 * Tests a small sample corpus, having some meta annotations, annotation
	 * definitions and one segments including one graph.
	 * 
	 * @throws IOException
	 */
	@Test
	...
	/**
	 * Tests a small sample corpus, having some meta annotations, annotation
	 * definitions and one segments including one graph. Checks only the correct
	...
	public void testReadSecEdges() throws IOException {
		...
		assertNotNull(corpus.findFeature(DEFAULT_TYPE.LABEL.toString(), DOMAIN.EDGE, DEFAULT_TYPE.PRIM.toString())
				.getFeatureValues());
		assertEquals(5, corpus.findFeature(DEFAULT_TYPE.LABEL.toString(), DOMAIN.EDGE, DEFAULT_TYPE.PRIM.toString())
				.getFeatureValues().size());

		assertNotNull(corpus.findFeature(DEFAULT_TYPE.LABEL.toString(), DOMAIN.EDGE, DEFAULT_TYPE.SEC.toString())
				.getFeatureValues());
		assertEquals(1, corpus.findFeature(DEFAULT_TYPE.LABEL.toString(), DOMAIN.EDGE, DEFAULT_TYPE.SEC.toString())
				.getFeatureValues().size());
