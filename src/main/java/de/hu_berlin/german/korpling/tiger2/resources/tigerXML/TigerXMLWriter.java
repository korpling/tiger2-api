...
public class TigerXMLWriter {
	 ...
	 * {@link TigerXMLDictionary#ELEMENT_CORPUS}).
	 * 
	 * @param corpus
	 ...
	 * {@link TigerXMLDictionary#ELEMENT_SUB_CORPUS}).
	 * 
	 * @param corpus
	 *            corpus to store as sub corpus
	 */
	protected void saveSubCorpus(Corpus corpus) {
		this.output.println("<" + TigerXMLDictionary.ELEMENT_SUB_CORPUS + " " + TigerXMLDictionary.ATTRIBUTE_ID + "=\""
				+ corpus.getId() + "\">");
		if (corpus.getMeta() != null)
			this.saveMeta(corpus.getMeta());
		if ((corpus.getSubCorpora() != null) && (corpus.getSubCorpora().size() > 0)) {// corpus
	 ...
	 * {@link TigerXMLDictionary#ELEMENT_META}.
	 * 
	 * @param meta
	...
	protected void saveMeta(Meta meta) {
		...
		if ((meta.getName() != null) && (!meta.getName().isEmpty()))
			this.output.println("<" + TigerXMLDictionary.ELEMENT_NAME + ">" + meta.getName() + "</"
					+ TigerXMLDictionary.ELEMENT_NAME + ">");
		if ((meta.getAuthor() != null) && (!meta.getAuthor().isEmpty()))
			this.output.println("<" + TigerXMLDictionary.ELEMENT_AUTHOR + ">" + meta.getAuthor() + "</"
					+ TigerXMLDictionary.ELEMENT_AUTHOR + ">");
		if ((meta.getDate() != null) && (!meta.getDate().isEmpty()))
			this.output.println("<" + TigerXMLDictionary.ELEMENT_DATE + ">" + meta.getDate() + "</"
					+ TigerXMLDictionary.ELEMENT_DATE + ">");
		if ((meta.getDescription() != null) && (!meta.getDescription().isEmpty()))
			this.output.println("<" + TigerXMLDictionary.ELEMENT_DESCRIPTION + ">" + meta.getDescription() + "</"
					+ TigerXMLDictionary.ELEMENT_DESCRIPTION + ">");
		if ((meta.getFormat() != null) && (!meta.getFormat().isEmpty()))
			this.output.println("<" + TigerXMLDictionary.ELEMENT_FORMAT + ">" + meta.getFormat() + "</"
					+ TigerXMLDictionary.ELEMENT_FORMAT + ">");
		if ((meta.getHistory() != null) && (!meta.getHistory().isEmpty()))
			this.output.println("<" + TigerXMLDictionary.ELEMENT_HISTORY + ">" + meta.getHistory() + "</"
					+ TigerXMLDictionary.ELEMENT_HISTORY + ">");
		this.output.println("</" + TigerXMLDictionary.ELEMENT_META + ">");
	}

	/**
	 * Stores the given list of {@link Feature} objects as (see
	 * {@link TigerXMLDictionary#ELEMENT_FEATURE}) elements.
	 * 
	 * @param corpus
	 *            corpus to store as sub corpus
	 */
	protected void saveFeatures(EList<Feature> features) {
		if (features != null) {
			...
			for (Feature feature : features) {
				if (DOMAIN.EDGE.equals(feature.getDomain())) {// compute String
																// for xml
																// element,
																// feature in
																// case of t or
					...
					if ((feature.getFeatureValues() != null) || (feature.getFeatureValues().size() > 0)) {
						...
						this.output.println(">");
				} // compute String for xml element, feature in case of t or nt,
					// edgelabel or secedgelabel in case of edge
				else {// print normal feature element
					...
					if (feature.getDomain() != null)
						this.output.print(" " + TigerXMLDictionary.ATTRIBUTE_DOMAIN + "=\""
								+ feature.getDomain().getName() + "\"");
	 ...
	 * {@link TigerXMLDictionary#ELEMENT_VALUE}) elements.
	 * 
	 * @param corpus
	 *            corpus to store as sub corpus
	 * @param output
	 ...
	 * {@link TigerXMLDictionary#ELEMENT_SEGMENT} elements inside a
	...
	protected void saveBody(EList<Segment> segments) {
		...
		if ((segments != null) && (segments.size() > 0)) {
			for (Segment segment : segments) {
				this.output.println("<" + TigerXMLDictionary.ELEMENT_SEGMENT + " " + TigerXMLDictionary.ATTRIBUTE_ID
						+ "=\"" + segment.getId() + "\">");
	 ...
	 * {@link TigerXMLDictionary#ELEMENT_GRAPH} elements inside a
	 ...
	 * {@link TigerXMLDictionary#ELEMENT_TERMINAL} elements inside a
	...
	protected void saveTerminal(Terminal terminal) {
		if (terminal != null) {
			this.output.print("<" + TigerXMLDictionary.ELEMENT_TERMINAL + " " + TigerXMLDictionary.ATTRIBUTE_ID + "=\""
					+ terminal.getId() + "\"");
	 ...
	 * {@link TigerXMLDictionary#ELEMENT_NONTERMINAL} elements inside a
	...
	protected void saveNonTerminal(NonTerminal nonTerminal) {
		if (nonTerminal != null) {
			this.output.print("<" + TigerXMLDictionary.ELEMENT_NONTERMINAL + " " + TigerXMLDictionary.ATTRIBUTE_ID
					+ "=\"" + nonTerminal.getId() + "\"");
	 ...
	 * {@link TigerXMLDictionary#ELEMENT_EDGE}.
	 * 
	 * @param edge
	 */
	protected void saveEdge(Edge edge) {
		...
		if (edge.getType() != null) {
			if ((DEFAULT_TYPE.PRIM.toString().equalsIgnoreCase(edge.getType()))
				...
				xmlElement = TigerXMLDictionary.ELEMENT_SECEDGE;
		} else {
			...
			for (Annotation annotation : edge.getAnnotations()) {
				if ((annotation.getFeatureRef().getType() != null)
					...
					if (featType == null)
						...
						throw new TigerInvalidModelException(
								"Cannot export <tiger2/> model to TigerXML, because the type of Annotation of the given edge '"
										+ edge.getId() + "' is not the same for all annotations.");
				}
			}
		...
		// start: print only 'label'-annotation, in case of no label annotation
		// exists, print first annotation
		...
		if (labelAnno != null)
			this.output.print(" " + DEFAULT_TYPE.LABEL.toString() + "=\""
					+ StringEscapeUtils.escapeXml(labelAnno.getValue()) + "\"");
		// end: print only 'label'-annotation, in case of no label annotation
		// exists, print first annotation
