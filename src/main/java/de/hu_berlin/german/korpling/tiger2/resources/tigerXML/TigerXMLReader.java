...
public class TigerXMLReader extends DefaultHandler2 {
	 ...
	 * {@link TigerXMLReader} object is created or in case such an object shall
	 * be reset for starting a new parsing. All internal sets, lists, variables
	 * and so on will be set to their initial value.
	 */
	public void init() {
	...
	public void characters(char[] ch, int start, int length) throws SAXException {
		if ((this.elementStack != null) && (this.elementStack.size() > 0)) {// only
			...
			} else if (TigerXMLDictionary.ELEMENT_VALUE.equals(this.elementStack.peek())) {// set
																							// description
																							// of
																							// feature
	...
	public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
		if (TigerXMLDictionary.ELEMENT_CORPUS.equals(qName)) {
		...
		} else if (TigerXMLDictionary.ELEMENT_META.equals(qName)) {// corpus/meta
			Meta meta = Tiger2Factory.eINSTANCE.createMeta();
			this.currentCorpus.setMeta(meta);
			elementStack.push(qName);
		} // corpus/meta
		else if (TigerXMLDictionary.ELEMENT_FEATURE.equals(qName)) {// corpus/annotation/feature
			if (TigerXMLDictionary.ELEMENT_ANNOTATION.toString().equals(elementStack.peek())) {
				...
				throw new TigerImplausibleContentException("An element of type '" + TigerXMLDictionary.ELEMENT_FEATURE
						+ "' was found, which is not nedted in a '" + TigerXMLDictionary.ELEMENT_ANNOTATION
						+ "' element");
		} // corpus/annotation/feature
		else if ((TigerXMLDictionary.ELEMENT_EDGELABEL.equals(qName))
				|| (TigerXMLDictionary.ELEMENT_SECEDGELABEL.equals(qName))) {// corpus/annotation/edgelabel
																				// or
																				// corpus/annotation/secedgelabel
			if (TigerXMLDictionary.ELEMENT_ANNOTATION.toString().equals(elementStack.peek())) {
		...
		else if (TigerXMLDictionary.ELEMENT_VALUE.equals(qName)) {// corpus/annotation/feature/value
			if ((TigerXMLDictionary.ELEMENT_FEATURE.toString().equals(this.elementStack.peek()))
				...
				throw new TigerImplausibleContentException("An element of type '" + TigerXMLDictionary.ELEMENT_VALUE
						+ "' was found, which is not contained in a '" + TigerXMLDictionary.ELEMENT_FEATURE
						+ "' element.");
			}
			...
			importNonTerminal(attributes);
				...
				|| (TigerXMLDictionary.ELEMENT_SECEDGE.equals(qName))) {// ELEMENT_EDGE
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			if (TigerXMLDictionary.ATTRIBUTE_ID.equals(attributes.getQName(i)))
				...
				if ((attName == null) || (attName.isEmpty()))
					throw new TigerImplausibleContentException(
							"An edge was found with an empty '" + TigerXMLDictionary.ATTRIBUTE_IDREF + "' value.");
	 ...
	 * {@link TigerXMLDictionary#ELEMENT_CORPUS} ends, all not yet targeted
	 * edges will be targeted and stored in the graph by calling
